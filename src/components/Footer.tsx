    ...
    <footer className="bg-[#1c1d1f] text-gray-300 w-full mt-auto border-t-4 border-[#af101a]">
      <div className="w-full py-12 px-6 max-w-[1440px] mx-auto flex flex-col md:flex-row justify-between gap-10">
        ...
        <div className="flex flex-wrap gap-10 lg:gap-16">
          ...
          {/* ADMIN PORTAL SECTION - Shown only when logged in as Administrator */}
          {isAdmin && (
            <div className="flex flex-col gap-2 min-w-[200px] bg-gray-900/80 p-4 rounded-xl border border-gray-800">
              <div className="flex items-center justify-between mb-1">
                <h4 className="font-bold text-xs text-[#af101a] uppercase tracking-wider flex items-center gap-1.5">
                  <span className="material-symbols-outlined text-[18px]">admin_panel_settings</span>
                  Admin Portal
                </h4>
                <span className="bg-[#af101a]/20 text-[#ffb3ac] border border-[#af101a]/40 text-[9px] font-extrabold px-1.5 py-0.5 rounded uppercase">
                  Personnel Only
                </span>
              </div>
              ...
              <button
                ...
                className={`text-left text-sm transition-all flex items-center gap-1.5 font-bold ${
                  activeTab?.startsWith('admin-')
                    ? 'text-[#ffb3ac] translate-x-1'
                    : 'text-gray-200 hover:text-[#ffb3ac]'
                }`}
              >
                🛡️ Admin Dashboard Portal
              </button>
              <button
                onClick={() => setActiveTab?.('admin-kerala-map')}
                className="text-left text-xs text-gray-400 hover:text-white transition-colors"
              >
                • Manage Relief Camps & Map
              </button>
              <button
                onClick={() => setActiveTab?.('admin-disaster-reports')}
                className="text-left text-xs text-gray-400 hover:text-white transition-colors"
              >
              ...
              <button
                ...
                className="text-left text-xs text-gray-400 hover:text-white transition-colors"
              >
                • Resource & Supplies Stock
              </button>
              <button
                onClick={() => setActiveTab?.('admin-volunteers')}
                className="text-left text-xs text-gray-400 hover:text-white transition-colors"
              >
                • Volunteer Roster & Deployment
              </button>
              <button
